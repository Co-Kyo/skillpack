...
export function resolveStepRefs(
  ...
  return text.replace(/\{\{step:([A-Za-z0-9_-]+)\}\}/g, (_, id: string) => {
    const seq = stepOrder[id];
    if (seq === undefined) {
      throw new Error(`Unresolved step reference: {{step:${id}}}`);
    }
    return `Step ${String(seq).padStart(2, '0')}`;
  });
}
...
export function renderStep(step: ResolvedStep): string {
  if (step.graph.kind === 'task' && step.graph.task.bodyFile && !step.body) {
    return resolveTaskBody(step.graph.task);
  }

  const stepBody = step.body ?? (step.bodyFile ? resolveBodyFile(step.bodyFile) : '');
  if (stepBody) {
    ...
    JSON.stringify(
      {
        ...
        source_commit: process.env.SKILLPACK_SOURCE_COMMIT ?? null,
